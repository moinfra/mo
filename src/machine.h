...
class PressureTracker
{
    ...
    std::map<unsigned, unsigned> pressure_at_pos_; // 使用有序映射以便遍历
    mutable bool max_pressure_dirty_ = false;      // 是否需要重新计算最大压力标志位
...
public:
    explicit PressureTracker(const TargetRegisterInfo &tri) : tri_(tri) {}
    void add_interval(const LiveRange &lr);
    void remove_interval(const LiveRange &lr);
...
public:
    ...
    static MOperand create_mem_rix(unsigned base_reg, unsigned index_reg,
    ...
    static MOperand create_frame_index(int idx);

    static MOperand create_global(GlobalVariable *gv);
...
public:
    ...
    std::string to_string(const TargetInstInfo *tii = nullptr) const;
...
public:
    ...
    MachineInst(unsigned opcode, const std::vector<MOperand> &ops = {});

    bool verify(VerificationLevel level, const TargetInstInfo *tii = nullptr,
    ...
    // Operand management
    void add_operand(const MOperand &op);
    void insert_operand(unsigned idx, const MOperand &op);
    void remove_operand(unsigned idx);
...
public:
    ...
    void add_successor(MachineBasicBlock *succ);

    void remove_successor(MachineBasicBlock *succ);
...
private:
    ...
    // 全局指令位置映射：指令指针 -> 全局位置
    mutable std::unordered_map<const MachineInst *, unsigned> global_instr_positions_;
    ...
    int create_frame_object(FrameObjectInfo info);

    // Get stack object information
    const FrameObjectInfo *get_frame_object(int idx) const;
...
public:
    ...
    explicit MachineFunction(Function *f) : ir_func_(f) {
        AliasCheckFn is_alias_fn = [this](unsigned reg1, unsigned reg2) {
            return false; // TODO: use info given by target
        };
        lra_ = std::unique_ptr<LiveRangeAnalysis>(new LiveRangeAnalysis(*this, is_alias_fn));
    }

    MachineBasicBlock *create_block();
    std::unique_ptr<PressureTracker> compute_pressure() const;
    unsigned create_vreg(unsigned rc_id, unsigned size = 4, bool is_fp = false,
                         Value *orig = nullptr);
    ...
    bool has_frame_index(Value *v) const;

    // Frame index management
    int create_frame_object(Value *v);
    int get_frame_index(Value *v) const;
    ...
    // 标记全局位置需要重新计算
    void mark_global_positions_dirty()
    {
        global_positions_dirty_ = true;
    }

    // 获取指令的全局位置
    unsigned get_global_instr_pos(const MachineInst *inst) const
    {
        assert(!global_positions_dirty_ && "Global positions not computed!");
        ensure_global_positions_computed();
        return global_instr_positions_.at(inst);
    }
...
public:
    ...
    virtual void analyze_call(MachineFunction &mf,
                              ...
                              std::vector<ArgLocation> &locs) const = 0;
    virtual void analyze_return(MachineFunction &mf, const Value *ret_val,
                                RetLocation &loc) const = 0;
};

struct RegisterDesc
{
    unsigned int spill_cost; // 寄存器溢出成本
    bool is_callee_saved;    // 被调用者保存
    bool is_reserved;        // 是否保留寄存器
    bool is_allocatable;     // 是否可被分配
    unsigned primary_rc_id;  // 新增主类ID
};
...
{
    std::string name_;
    std::vector<unsigned> regs; // 寄存器列表
    unsigned copy_cost;         // 寄存器间复制成本
...
protected:
    ...
    // 基于调用约定的 Callee/Caller Saved Registers 预存
...
public:
    explicit TargetRegisterInfo(unsigned numRegs)
        : reg_descs_(numRegs), alias_map_(numRegs) {}

    //===---------------- 寄存器属性查询 -------------------===//
    bool is_callee_saved(unsigned reg) const
    {
        return reg_descs_[reg].is_callee_saved;
    }

    bool is_reserved_reg(unsigned reg) const
    {
        return reg_descs_[reg].is_reserved;
    }

    unsigned get_spill_cost(unsigned reg) const
    {
        return reg_descs_[reg].spill_cost;
    }
    virtual unsigned get_primary_reg_class(unsigned reg) const
    {
        return reg_descs_[reg].primary_rc_id;
    }
    void add_register_class(const RegisterClass &rc)
    {
        register_classes_.push_back(std::make_unique<RegisterClass>(rc));
    }

    RegisterClass *get_reg_class(unsigned rc_id) const
    {
        if (rc_id >= register_classes_.size())
            return nullptr;

        RegisterClass *rc = register_classes_[rc_id].get();
        return rc;
    }

    unsigned get_reg_class_weight(unsigned rc_id) const
    {
        const RegisterClass *rc = get_reg_class(rc_id);
        return rc ? rc->weight : 1; // 默认权重为1
    }
...
public:
    ...
    virtual bool is_legal_immediate(int64_t imm,
    ...
    virtual void copy_phys_reg(MachineBasicBlock &mbb,
                               ...
                               unsigned dest_reg, unsigned src_reg) const = 0;

    // Legalization handling
    virtual bool legalize_inst(MachineInst &mi, MachineFunction &mf) const = 0;
};
...
public:
    ...
    virtual int get_frame_index_offset(const MachineFunction &mf, int fi) const = 0;
    ...
    virtual void emit_stack_protector(MachineFunction &mf,
                                      int guard_idx) const = 0; // Insert stack protection code
};
...
public:
    explicit MachineModule(Module *m) : ir_module_(m) {}

    MachineFunction *create_machine_function(Function *f);

    void set_target_info(const TargetRegisterInfo *tri,
