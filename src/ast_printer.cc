...
string ASTPrinter::print(const Expr& expr) {
    if (auto p = dynamic_cast<const VariableExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const IntegerLiteralExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const FloatLiteralExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const StringLiteralExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const BinaryExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const UnaryExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const CallExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const MemberAccessExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const CastExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const SizeofExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const InitListExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const FunctionPointerExpr*>(&expr)) return visit(*p);
    if (auto p = dynamic_cast<const StructLiteralExpr*>(&expr)) return visit(*p);
    return "unknown_expr";
}

string ASTPrinter::print(const Statement& stmt) {
    if (auto p = dynamic_cast<const BlockStmt*>(&stmt)) return visit(*p);
    if (auto p = dynamic_cast<const VarDeclStmt*>(&stmt)) return print(*p);
    if (auto p = dynamic_cast<const GlobalDecl*>(&stmt)) return print(*p);
    if (auto p = dynamic_cast<const ReturnStmt*>(&stmt)) return visit(*p);
    if (auto p = dynamic_cast<const IfStmt*>(&stmt)) return visit(*p);
    if (auto p = dynamic_cast<const WhileStmt*>(&stmt)) return visit(*p);
    if (auto p = dynamic_cast<const BreakStmt*>(&stmt)) return visit(*p);
    if (auto p = dynamic_cast<const ContinueStmt*>(&stmt)) return visit(*p);
    if (auto p = dynamic_cast<const StructDecl*>(&stmt)) return print(*p);
