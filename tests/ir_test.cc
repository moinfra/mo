...
TEST(TruncateValueTest, UnsignedTruncation)
{
    ...
    ASSERT_EQ(truncate_value(0xFFFFFFFFFFFFFFFF, 8, true), 0xFF);
    ASSERT_EQ(truncate_value(0x123456789ABCDEF0, 16, true), 0xDEF0);
    ASSERT_EQ(truncate_value(0x123456789ABCDEF0, 32, true), 0x9ABCDEF0);
    ASSERT_EQ(truncate_value(0x123456789ABCDEF0, 64, true), 0x123456789ABCDEF0);
    ASSERT_EQ(truncate_value(0x123, 0, true), 0x123); // Case where bit_width = 0
}

TEST(TruncateValueTest, SignedTruncationPositive)
{
    // Test signed truncation, positive numbers
    ASSERT_EQ(truncate_value(0x7F, 8, false), 0x7F);
    ASSERT_EQ(truncate_value(0x7FFF, 16, false), 0x7FFF);
    ASSERT_EQ(truncate_value(0x7FFFFFFF, 32, false), 0x7FFFFFFF);
}

TEST(TruncateValueTest, SignedTruncationNegative)
{
    // Test signed truncation, negative numbers
    ASSERT_EQ(truncate_value(0x80, 8, false), static_cast<uint64_t>(0xFFFFFFFFFFFFFF80));
    ASSERT_EQ(truncate_value(0x8000, 16, false), static_cast<uint64_t>(0xFFFFFFFFFFFF8000));
    ASSERT_EQ(truncate_value(0x80000000, 32, false), static_cast<uint64_t>(0xFFFFFFFF80000000));
}

TEST(TruncateValueTest, SignedTruncationEdgeCases)
{
    // Test signed truncation, edge cases
    ASSERT_EQ(truncate_value(0x00, 8, false), 0x00);                                      // Minimum value
    ASSERT_EQ(truncate_value(0xFF, 8, false), static_cast<uint64_t>(0xFFFFFFFFFFFFFFFF)); // Maximum value
    ASSERT_EQ(truncate_value(0x8000000000000000, 64, false), 0x8000000000000000);         // 64-bit negative minimum
    ASSERT_EQ(truncate_value(0x7FFFFFFFFFFFFFFF, 64, false), 0x7FFFFFFFFFFFFFFF);         // 64-bit positive maximum
}
...
TEST(TruncateValueTest, VariousInputs)
{
    ...
    ASSERT_EQ(truncate_value(0x10, 5, true), 0x10);
    ASSERT_EQ(truncate_value(0x10, 5, false), static_cast<uint64_t>(0xFFFFFFFFFFFFFFF0));
    ASSERT_EQ(truncate_value(0x1F, 5, false), static_cast<uint64_t>(0xFFFFFFFFFFFFFFFF));
}

TEST(TruncateValueTest, BitsEqualTo64)
{
    // Test the case where bit_width is equal to 64
    ASSERT_EQ(truncate_value(0xFFFFFFFFFFFFFFFF, 64, true), 0xFFFFFFFFFFFFFFFF);
    ASSERT_EQ(truncate_value(0x8000000000000000, 64, false), 0x8000000000000000);
    ASSERT_EQ(truncate_value(0x7FFFFFFFFFFFFFFF, 64, false), 0x7FFFFFFFFFFFFFFF);
    ASSERT_EQ(truncate_value(0x123456789ABCDEF0, 64, true), 0x123456789ABCDEF0);
}
